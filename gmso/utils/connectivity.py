...
def identify_connections(top, index_only=False):
    ...
    print("------DIHEDRALS FOUND-------------")
    for dih in dihedral_matches:
        print(dih)
    print("------IMPROPERS FOUND-------------")
    for imp in improper_matches:
        print(imp)
        ...
        for c in b_neighbors:
            ...
            for a in b_neighbors:
                ...
                for d in adj[c]:
                    if d == b:
                        continue
                    if d == a:          # <-- missing guard: catches cycles where
                        continue        #     start and end would be the same atom
                    if a < d:
                        matches.add((a, b, c, d))
                    else:
                        matches.add((d, c, b, a))

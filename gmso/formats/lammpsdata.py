...
def _write_impropertypes(out_file, top, base_unyts, parser, cfactorsDict):
    ...
    improper_typesList = []
    if parser.__name__ in (
        "parse_cvff_style_improper",
        "parse_harmonic_style_improper",
    ):
        ...
        for improper_type, members in index_membersList:
            ...
            variable_msg = "{:8}\t" * len(parameterStrList)
            full_msg = base_msg + variable_msg + end_msg
            for parameter_terms in parameter_termList:
                out_file.write(
                    full_msg.format(
                        idx + 1,
                        *[
                ...
                improper_typesList.append(improper_type)
                idx += 1
...
def parse_harmonic_style_improper(improper_type):
    ...
    phi_eqArray = improper_type.parameters["phi_eq"].flatten()
    allParamsList = []
    for a, c in zip(kArray, phi_eqArray):
        allParamsList.append([a, c])
